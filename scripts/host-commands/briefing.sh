...
docker exec homelab-prefect-worker-1 ssh -o BatchMode=yes lucille5 'export PATH=/opt/homebrew/bin:$PATH && set -a && source ~/.config/heartbeat/.env && set +a && cd ~/git/obsidian-tools && .venv/bin/python src/personal/briefing.py 2>/dev/null'
